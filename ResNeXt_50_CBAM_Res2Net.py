...
class Bottleneck(nn.Module):
    def __init__(self, bottleneck_in_channels, bottleneck_out_channels_quarter, downsample=None, stride_num=1):
        super(Bottleneck, self).__init__()
        self.width = bottleneck_out_channels_quarter*2//4
        self.conv1 = nn.Conv2d(bottleneck_in_channels, bottleneck_out_channels_quarter*2, kernel_size=(1, 1), stride=(1, 1), bias=False)
        self.bn1 = nn.BatchNorm2d(bottleneck_out_channels_quarter*2, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        # self.conv2 = nn.Conv2d(bottleneck_out_channels_quarter*2, bottleneck_out_channels_quarter*2, kernel_size=(3, 3), stride=(stride_num, stride_num), padding=(1, 1), groups=32, bias=False)
        # 3 conv2 layer, groups=8
        self.conv2 = nn.ModuleList([
            nn.Conv2d(self.width, self.width, kernel_size=(3, 3), stride=(stride_num, stride_num), padding=1, groups=32//4, bias=False)
            for _ in range(3)
        ])
        # self.bn2 = nn.BatchNorm2d(bottleneck_out_channels_quarter*2, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        # one bn3 for each split
        self.bn2 = nn.ModuleList([nn.BatchNorm2d(self.width, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True) for _ in range(3)])
        self.conv3 = nn.Conv2d(bottleneck_out_channels_quarter*2, bottleneck_out_channels_quarter*4, kernel_size=(1, 1), stride=(1, 1), bias=False)
        self.bn3 = nn.BatchNorm2d(bottleneck_out_channels_quarter*4, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.cbam = CBAM(bottleneck_out_channels_quarter*4)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride_num = stride_num
        self.pool = nn.AvgPool2d(kernel_size=3, stride=(stride_num, stride_num), padding=1)
    ...
    def forward(self, x):
        ...
        # x = self.conv2(x)
        # x = self.bn2(x)
        xs = torch.chunk(x, len(self.conv2) + 1, dim=1)
        # first split
        if self.stride_num == 2:  # special case
            ys = [self.pool(xs[0])]
        else:
            ys = [xs[0]]
        # second~forth split
        for i in range(len(self.conv2)): 
            if self.stride_num == 2:  # special case
                ys.append(self.relu(self.bn2[i](self.conv2[i](xs[i + 1]))))
            else:
                ys.append(self.relu(self.bn2[i](self.conv2[i](ys[-1] + xs[i + 1]))))
        # concat four splits
